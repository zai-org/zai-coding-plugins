  ...
  "node_modules/readdir-glob/index.js"(exports2, module2) {
    ...
    async function* exploreWalkAsync(dir, path2, followSymlinks, useStat, shouldSkip, strict) {
      ...
      for (const file of files) {
        ...
        const absolute = path2 + "/" + relative3;
        let stats = null;
...
var getPackageVersion = () => {
  try {
    return JSON.parse((0, import_fs.readFileSync)((0, import_path.join)(process.cwd(), "package.json"), "utf-8"));
  } catch (error) {
    console.error("Error reading package.json:", error);
  }
};
var updateDeploySettings = (newSettings) => {
  try {
    (0, import_fs.mkdirSync)((0, import_path.join)(process.cwd(), ".deploy"), { recursive: true });
    const settingsString = JSON.stringify(newSettings);
    (0, import_fs.writeFileSync)((0, import_path.join)(process.cwd(), ".deploy/settings.json"), settingsString, "utf-8");
  } catch (error) {
    console.error("Error writing .deploy/settings.json:", error);
  }
};
var loadDeploySettings = () => {
  try {
    const projectName = (0, import_path.basename)(process.cwd());
    const settingsPath = (0, import_path.join)(process.cwd(), ".deploy/settings.json");
    let deploySettings;
    try {
      deploySettings = JSON.parse((0, import_fs.readFileSync)(settingsPath, "utf-8"));
    } catch (error) {
    ...
    try {
      ...
      envLines = (0, import_fs.readFileSync)(envPath, "utf-8");
    } catch (error) {
    ...
    updateDeploySettings(deploySettings);
...
async function zipFiles(targetFile, files) {
  return new Promise((resolve2, reject) => {
    const output = (0, import_fs.createWriteStream)(targetFile);
    const archive = (0, import_archiver.default)("zip", {
      zlib: { level: 9 }
      // Sets the compression level
    });
    output.on("close", () => {
      resolve2(new File([(0, import_fs.readFileSync)(targetFile)], (0, import_path.basename)(targetFile), { type: "application/zip" }));
    });
    ...
    for (const filePath of files) {
      if ((0, import_fs.existsSync)(filePath)) {
        const relativePath = (0, import_path.relative)(process.cwd(), filePath);
...
function MD5(envLines) {
  return (0, import_crypto.createHash)("md5").update(envLines).digest("hex");
}
...
var FileFilter = class {
   ...
   * 简单的模式匹配（作为正则表达式的后备）
   */
  simplePatternMatch(pattern, path2) {
    if (pattern === path2) return true;
    if (pattern.endsWith("/")) {
      return path2.startsWith(pattern) || path2 === pattern.slice(0, -1);
    }
    if (pattern.includes("*")) {
...
var uploadAndDeploy = async (file, env, projectId) => {
...
var getDeployStatus = async (projectId, deploymentId) => {
...
var handleHttpResponse = async (response) => {
...
// index.ts
var programDir = (0, import_path2.dirname)(process.argv[1]);
(0, import_process.loadEnvFile)(`${programDir}/.env`);
var packageJson = getPackageVersion();
...
async function deploySkill(ignoreFilePath) {
  try {
    const { deploySettings, env } = await loadDeploySettings();
    console.info("Deploy settings:", deploySettings);
    console.info("Environment variables:", env);
    const localFolder = process.cwd();
    const files = await filterDirectoryFiles(localFolder, ignoreFilePath);
    console.info(`filtered files count: ${files.length}`);
    const tmpDir = (0, import_path2.join)(localFolder, ".tmp");
    if (!(0, import_fs2.existsSync)(tmpDir)) {
      (0, import_fs2.mkdirSync)(tmpDir, { recursive: true });
    }
    const tmpZipFn = (0, import_path2.join)(tmpDir, `${Date.now()}_upload.zip`);
    const file = await zipFiles(tmpZipFn, files);
    let projectId = null;
    let deploymentId = null;
    try {
      console.info("Start to upload and deploy");
      const deploymentResult = await uploadAndDeploy(file, env, deploySettings.projectId);
    ...
    await updateDeploySettings(deploySettings);
    let status = { status: "Pending", id: deploymentId };
    while (true) {
      status = await getDeployStatus(deploySettings.projectId, deploymentId);
...
async function statusSkill() {
  try {
    ...
    const status = await getDeployStatus(deploySettings.projectId, deploySettings.deployments[0].deploymentId);
    return {
      success: true,
...
async function zipSkill(targetPath, ignoreFilePath) {
  try {
    const zip = await zipFiles(targetPath, await filterDirectoryFiles(process.cwd(), ignoreFilePath));
    return {
      success: true,
...
async function testSkill() {
  try {
    const folderList = await filterDirectoryFiles(process.cwd());
  ...
  switch (command) {
    case "deploy": {
      ...
      if (result.success) {
        ...
        console.log("\u{1F4CD} URL:", result.url);
      } else {
    ...
    case "status": {
      ...
      if (result.success) {
        ...
        console.log("\u{1F4CD} status:", result.status);
      } else {
    ...
    case "zip": {
      const targetPath = args[1] || (0, import_path2.join)(process.cwd(), "output.zip");
    ...
    case "test": {
      ...
      if (result.success) {
        ...
        console.log("\u{1F4CD} Message:", result.message);
      } else {
